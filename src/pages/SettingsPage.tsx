...
const SettingsPage = () => {
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    // Normal mode: re-authenticate with current password first
    if (!currentPassword) {
  ...
  return (
    <div className="space-y-6 relative z-10 px-4 py-6">
      ...
      <div className="bg-black/30 backdrop-blur-md border border-primary/20 rounded-xl p-6 max-w-2xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <div className="w-10 h-10 rounded-lg bg-primary/20 flex items-center justify-center"><KeyRound size={20} className="text-primary" /></div>
        ...
        <form onSubmit={handlePasswordChange} className="space-y-4">
          {!isRecoveryMode && (
            <div>
              ...
              <Input type="password" value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} required placeholder="Enter current password" className="bg-black/40 border-primary/20" />
            </div>
          )}
          <div>
            <label className="block text-sm font-medium text-foreground/80 mb-2">New Password</label>
            <Input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} required placeholder="Min 6 characters" className="bg-black/40 border-primary/20" />
          </div>
          <div>
            <label className="block text-sm font-medium text-foreground/80 mb-2">Confirm New Password</label>
            <Input type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} required placeholder="Re-enter new password" className="bg-black/40 border-primary/20" />
          </div>
